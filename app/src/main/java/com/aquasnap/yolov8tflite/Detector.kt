...
class Detector(
    ...
    private val labelPath: String, // Path to labels file in assets (e.g., "labelsinfo.txt")
    private val detectorListener: DetectorListener
) {

    private var interpreter: Interpreter? = null
    private var labels = mutableListOf<String>() // List to hold the labels
    private var descriptions = mutableMapOf<String, String>() // Map to hold descriptions
    ...
    fun setup() {
        ...
        // Load labels and descriptions from assets folder
    ...
    fun detect(frame: Bitmap) {
        ...
        if (bestBoxes == null) {
            // When no bounding boxes are detected, call onEmptyDetect
            detectorListener.onEmptyDetect()
            return
        }

        // Otherwise, pass the detected bounding boxes to the listener
    ...
    private fun bestBox(array: FloatArray): List<BoundingBox>? {
        ...
        for (c in 0 until numElements) {
            ...
            // Update the class name logic to include "Unknown - % Similar to - classname"
            ...
            // If the confidence is above the threshold, add the bounding box to the list
            if (maxConf > CONFIDENCE_THRESHOLD) {
                ...
                boundingBoxes.add(
                    BoundingBox(
                        ...
                        confidencePercentage = confidencePercentage // Pass the percentage here
                    )
                )
            }
        }
